<li class="hs-accordion" id="groups-accordion">
    <button type="button"
        class="cursor-pointer hs-accordion-toggle w-full text-start flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-gray-800 rounded-lg hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100 dark:bg-neutral-800 dark:hover:bg-neutral-700 dark:focus:bg-neutral-700 dark:text-neutral-200"
        ...
        @isset($groups)
            @foreach ($groups as $group)
                <ul class="relative hs-accordion-group ps-8 pt-1 space-y-1" data-hs-accordion-always-open>
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="#5D0E07" onclick="deleteGroup({{ $group->id }})" class="absolute left-2 top-3 cursor-pointer hover:fill-red-600">
                        <path d="M288-444h384v-72H288v72ZM480.28-96Q401-96 331-126t-122.5-82.5Q156-261 126-330.96t-30-149.5Q96-560 126-629.5q30-69.5 82.5-122T330.96-834q69.96-30 149.5-30t149.04 30q69.5 30 122 82.5T834-629.28q30 69.73 30 149Q864-401 834-331t-82.5 122.5Q699-156 629.28-126q-69.73 30-149 30Zm-.28-72q130 0 221-91t91-221q0-130-91-221t-221-91q-130 0-221 91t-91 221q0 130 91 221t221 91Zm0-312Z"></path>
                    </svg>

                    <li>
                        <button type="button"
                            class="cursor-pointer hs-accordion-toggles w-full text-start flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-gray-800 rounded-lg hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100 dark:bg-neutral-800 dark:hover:bg-neutral-700 dark:focus:bg-neutral-700 dark:text-neutral-200"
                            aria-expanded="false"
                            ...
                            <ul class="pt-1 space-y-1" id="user-list-{{ $group->id }}">
                                @foreach($group->users as $user)
                                    <li class="flex items-center gap-x-4 py-2 px-2.5">
                                        <a href="{{ route('users.show', $user->id) }}" class="user-link flex items-center w-full" draggable="true" data-user-id="{{ $user->id }}" data-profile-picture="./storage/{{ $user->profile_picture }}" data-first-name="{{ $user->first_name }}" data-last-name="{{ $user->last_name }}">
